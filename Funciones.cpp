...
Reclamo cargarReclamo(Usuario &usu) {
    ...
    int reclamoId = acum.getIdReclamos(); // Obtener el ID del próximo reclamo
    int contador = 0;
    ...
    if (contador == 0) {
        std::cout << "No tienes ningún suministro asociado sobre el cual puedas generar un reclamo.\n";
        return Reclamo(); // Devuelve un reclamo vacío si no hay suministros
    } else {
        ...
        if (suministroAGenerarReclamo.getSuministroId() != 0) {
            ...
            // Selección del tipo de reclamo
            seleccionarTipoReclamo(tipoDeReclamo);

            // Ingreso de la descripción
            std::cout << "Ingrese la descripción del reclamo: ";
            ...
            // ID del responsable de atención
            responsableDeAtencion = 1;

            // Respuesta vacía inicial
            strcpy(respuesta, "");
        ...
        system("cls");
        std::cout << "No tienes ningún suministro asociado con el ID ingresado.\n";
        return Reclamo(); // Devuelve un reclamo vacío si el ID no es válido
    }
}
        ...
        while (opcion != 0) {
            ...
            switch (opcion) {
                case 1:
                    ...
                    char respuesta[256]; // Tamaño fijo para la cadena de caracteres
...
// Función para intercambiar dos usuarios
...
void listadoUsuarios() {
    ...
    // Usar memoria dinámica para almacenar usuarios
...
void listadoSuministros() {
    ...
    // Usar memoria dinámica para almacenar suministros
...
void listadoFacturas() {
    ...
    // Usar memoria dinámica para almacenar facturas
...
void listadoPedidos() {
    ...
    // Usar memoria dinámica para almacenar pedidos
...
void listadoReclamos() {
    ...
    // Usar memoria dinámica para almacenar reclamos
...
// Función para encontrar clientes con más y menos facturas
void EstadisticaFacturas() {
    ...
    if (cantidadFacturasTotales == 0) {
        std::cout << "\nNo hay facturas registradas. Todos los usuarios están sin facturas." << std::endl;
        return;
    }

    // Usar memoria dinámica para almacenar usuarios y facturas
    Usuario* usuarios = new Usuario[cantidadUsuariosTotales];
    Factura* facturas = new Factura[cantidadFacturasTotales];
    ...
    // Encontrar el usuario con más y menos facturas
    for (int i = 0; i < cantidadUsuariosTotales; ++i) {
    ...
    if (usuarioMaxFacturas == usuarioMinFacturas) {
        std::cout << "\nEl cliente con más facturas es el mismo que el cliente con menos facturas:\nID "
...
void EstadisticaDeuda() {
    ...
    // Usar memoria dinámica para almacenar usuarios y suministros
    Usuario* usuarios = new Usuario[cantidadUsuariosTotales];
    Suministro* suministros = new Suministro[cantidadSuministrosTotales];
